...
class PDAP:
    ...
    def solve_exact(self, tol: float, do_logging: True) -> dict:
        ...
        while Phi_value > tol:
            ...
            k += 1
        logging.info(
            f"Finite PDAP converged in {k} iterations and {time.perf_counter()-start_time:.3f}s to tolerance {tol:.3E} with final sparsity of {len(u.support)} and objective {objectives[-1]:.12E}"
        )
